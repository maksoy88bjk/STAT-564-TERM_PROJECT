...
Data period is between 01.01.2009 and 31.12.2023 for Mugla Station (17292). Elevation of the station is 646 meter. Daily precipitation data is used as response variable in modelling. Distinct daily observation variables are used as predictors such as temperature (CB0), dew point temperature (CB0), sea-level air pressure (mb), cloud coverage (0-8), relative humidity (%), solar exposure (hour), solar radiation (W/m^2^) direction of wind (B0) and speed (m/sec).
...
2.  **Temperature**: This is a measure of the warmth or coldness of the atmosphere as determined by a thermometer. It is typically reported in degrees Celsius.
    ...
    -   The temperature ranges from cold to hot conditions, with mean temperatures around 10-15 CB0 and maximum temperatures reaching as high as 41.7.
...
3.  **Dew Point Temperature**: This is the temperature at which air becomes saturated with moisture and dew can form. It is an indicator of the moisture content in the air and is measured in degrees Celsius.
    ...
    -   Dewpoint temperatures indicate varying levels of atmospheric moisture, ranging from dry to humid conditions with maximum 19.9 CB0.
...
ggplot(df_pre, aes(x= Date, y= Precipitation))+
       ...
       geom_smooth(method= "loess", se= FALSE, color= "orange", size= 1.5)+
       geom_point(aes(color= !is.na(Precipitation)), size= 1,alpha=0.5, shape= 21, fill= "white", show.legend= FALSE) +
       geom_text(data= subset(df_pre, is.na(Precipitation)), aes(label= "NA", y= 0), vjust= -0.5, color= "blue",size= 3) + 
...
According to the time series plot of the precipitation, there is no specific long-term trend (orange line) over the years. Since the majority of the precipitation in Mugla was recorded as zero, trend line is around 0 as well. 4 of the 5 null values are found sequentially and one of them is located in "2012-11-09". The plot exhibits high volatility and also might be observed some seasonality (clearly observed in plot 2).
...
Before contructing SARIMA& SARIMAX to the precipitation data we should follow some assumptions which are stationarity, seasonality, heteroscedasticity and normality in residuals.
...
Since our p-value is less than 0.05, it can be concluded that the time series is stationary. That means that the mean, variance and autocorrelation of the series do not change over the years. Since we satisfied the assumption of stationary we do not need to take first-order differencing in the data.
...
data.frame(Date= c(df_n$Date, df_n$Date),
            ...
            geom_line()+ theme_bw()+ scale_color_manual(name=" ",values= c("Actual"= "dodgerblue", "Predicted"= "firebrick"),
...
data.frame(Date= c(df_n_lag $Date, df_n_lag $Date),
                ...
                ggplot(aes(x= Date, y= Precipitation, color= Type)) + 
                geom_line() +theme_bw() +
                scale_color_manual(name=" ",values= c("Actual"= "dodgerblue","Predicted"= "firebrick"),
