---
title: "Daily Rainfall Modelling in one of the automated-weather observation station in Mugla Province of Turkiye"
author: 
...
Now we use the cv.tree() function to see whether pruning the tree will improve performance. K-fold cross validation technique is preformed to prune the regression tree. The minimum deviance with less complexity is available where number of terminal nodes is 4. Thus, we can prune the regression tree from 14 to 4 terminal nodes.
...
The test set MSE associated with the bagged regression tree is `r round(mean((yhat.bag - df_tree.test)^2),2)`, this is almost 20% lower than that achived using an optimally-pruned single tree.
...
The test MSE of boosted model obtained is `r round(mean((yhat.boost - df_tree.test)^2),2)` and it is superior to the test MSE for random forests and bagging.
